...
const { sendSlackMessage, sendEmail, buildSlackAlert } = require('./services/alertService');
const { createIssue } = require('./services/githubService');
const { saveAlert, printSummary } = require('./services/alertLogger');
...
async function runPipeline(opts = {}) {
  ...
  for (const alert of allAlerts) {
    const channels = { slack: false, email: false, github: false };
    ...
    try {
      ...
      console.log(`Created GitHub issue for ${alert.projectId} (${alert.date})`);
      channels.github = true;
    } catch (error) {
      console.warn('GitHub issue creation failed (check GITHUB_TOKEN/GITHUB_REPO):', error.message);
    }

    // Persist alert to local JSON log regardless of channel success/failure
    const record = saveAlert({ ...alert, channels });
    console.log(`Alert saved to log: ${record.id}`);
  }
